...
export class PasswordDirective implements OnDestroy, DoCheck {
    ...
    constructor(public config: PrimeNGConfig, @Inject(DOCUMENT) private document: Document, @Inject(PLATFORM_ID) private platformId: any, private renderer: Renderer2, public el: ElementRef, public zone: NgZone) {}
    ...
    template: `
        <div [ngClass]="toggleMask | mapper : containerClass" [ngStyle]="style" [class]="styleClass" [attr.data-pc-name]="'password'" [attr.data-pc-section]="'root'">
            <input
                ...
                [autofocus]="autofocus"
            />
            @if (showClear && value != null) { @if (!clearIconTemplate) {
            ...
            } @if (toggleMask) { @if (unmasked) { @if (!hideIconTemplate) {
            <EyeSlashIcon (click)="onMaskToggle()" [attr.data-pc-section]="'hideIcon'" />
            } @if (hideIconTemplate) {
            <span (click)="onMaskToggle()">
                <ng-template *ngTemplateOutlet="hideIconTemplate"></ng-template>
            </span>
            } } @if (!unmasked) { @if (!showIconTemplate) {
            <EyeIcon (click)="onMaskToggle()" [attr.data-pc-section]="'showIcon'" />
            } @if (showIconTemplate) {
            <span (click)="onMaskToggle()">
