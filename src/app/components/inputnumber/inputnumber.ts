...
export class InputNumber implements OnInit, AfterContentInit, OnChanges, ControlValueAccessor {
    ...
    getCurrencyExpression(): RegExp {
        if (this.currency) {
            const formatter = new Intl.NumberFormat(this.locale, { style: 'currency', currency: this.currency, currencyDisplay: this.currencyDisplay, minimumFractionDigits: 0, maximumFractionDigits: 0 } as Intl.NumberFormatOptions);
    ...
    getPrefixExpression(): RegExp {
        if (this.prefix) {
            ...
            const formatter = new Intl.NumberFormat(this.locale, { style: this.mode, currency: this.currency, currencyDisplay: this.currencyDisplay } as Intl.NumberFormatOptions);
    ...
    getSuffixExpression(): RegExp {
        if (this.suffix) {
            ...
            const formatter = new Intl.NumberFormat(this.locale, { style: this.mode, currency: this.currency, currencyDisplay: this.currencyDisplay, minimumFractionDigits: 0, maximumFractionDigits: 0 } as Intl.NumberFormatOptions);
